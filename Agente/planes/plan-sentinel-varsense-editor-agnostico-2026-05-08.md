...
## Restricciones operativas 085A-4

- No reinstalar la extension en este bloque: el plan esta a mitad de refactor y la validacion debe hacerse con compile, pruebas unitarias y smoke CLI.
- Sentinel debe detectar especificaciones de diseno CSS que recrean controles o submenus fuera del sistema, aunque el selector no contenga `boton` o `button`.
- Caso motivador: `.selectorIdiomaTrigger` define trigger interactivo y `.selectorIdiomaLista::before` define aparicion visual de submenu; ambos deben aparecer en reportes del core/CLI.
...
## Avance 2026-05-08 - especificaciones CSS locales

- `085A-4`: Sentinel agrego la regla core `css-especificacion-diseno-local` para detectar controles, triggers, opciones y dropdowns locales que recrean recetas visuales del sistema aunque no se llamen `boton`/`button`.
- `085A-4`: El caso motivador `LanguageSelector.css` queda cubierto: el CLI compilado detecta `.selectorIdiomaTrigger`, `.selectorIdiomaLista` y `.selectorIdiomaOpcion` como especificaciones visuales locales en el reporte JSON.
- `085A-4`: Se agrego fixture de equivalencia `fixtures/equivalence/language-selector-design-spec/` con esperado JSON y prueba core vs CLI para evitar divergencias futuras.
- Validacion: Sentinel `npm run compile` y `npm run test:unit` pasaron (`291 passing`, `1 pending`). Smoke CLI real con `node out/cli/index.js analyze --file ...LanguageSelector.css --format json` paso y mostro la nueva regla en el reporte.
